                       ...
                       @foreach ($request as $request)
                           <div class="col-lg-6">
                               <div class="d-flex align-items-center">
                                    <img class="flex-shrink-0 img-fluid rounded-full" src="{{ asset('storage/images/' . $request->item->image) }}"
                                 alt="" style="width: 80px;">
                                   <div class="w-100 d-flex flex-column text-start ps-4">
                                       <h5 class="d-flex justify-content-between border-bottom pb-2">
                                           <span>{{ $request->item->name }}</span>
                                           <span class="text-primary">{{ $request->item->price }} $</span>
                                       </h5>
                                       <small class="fst-italic"> {{ $request->item->description }}</small>

                                       <div class="d-flex justify-content-between align-items-center">
                                           <h5 class="text-primary ">By » {{ $request->user->name }} <br>
                                       <p class="text-danger size2"> {{ $request->user->email }}</p></h5>
                                           <p class="ml-auto text-dark size ">{{ $request->created_at->diffForHumans() }}</p>
                                       </div>
                                       <form action="{{ route('removeItem' , ['id'=>$request->id]) }}" method="post">
                                                @csrf
                                                <button name="addbtn" type="submit" class="btn btn-danger"">Remove</button>
                                                </form>
                                   </div>
                               </div>
                           </div>
